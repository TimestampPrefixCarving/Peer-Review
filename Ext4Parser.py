...
def extentDive(mapF, q, validInodeLoc, dirEntrySynchronizers, BGSize, bSize, partitionStart, tabs, pdt, md5Data, md5):
    ...
    if((extTest == 62218) and (depthTree == pdt - 1)):
        ...
        for i in range(numExt):
            ...
            if(depthTree > 0):
                ...
                #I will probably have to extend this in the future...
                #We haven't really figured out how to handle the recursion for the hash....
                extentDive(mapF, q, phyBlockLoc * bSize + partitionStart, dirEntrySynchronizers, BGSize, bSize, partitionStart, tabs + 1, depthTree, md5)
                ...
                if(md5):
                    ...
                    md5Data = mapF[ partitionStart + phyBlockLoc*bSize : partitionStart + phyBlockLoc*bSize + bSize*numBlocksInExt]  
...
def updateDirSynch(mmapF, relativeOffsetInode, bSize, partitionStart, preValue, extFlag):
    ...
    if(extFlag):
        ...
        #We assume first block has all dir entries.  
...
def printInodes(mapF, q, validInodeLoc, dirEntrySynchronizers, BGSize, bSize, partitionStart, inodeDict, fileVerDict, c, md5):
    ...
    #double check magic number
    if(extFlag and (bytesToDec(mapF[validInodeLoc + 40 : validInodeLoc + 42]) == 62218)):
        ...
        dataPtrs += " | Max Number of Extents: "
        ...
        for i in range(numExt):
            ...
            if(depthTree > 0):
                ...
                #If you actually need to go into the tree
                m = extentDive(mapF, q, phyBlockLoc * bSize + partitionStart, dirEntrySynchronizers, BGSize, bSize, partitionStart, 0, depthTree, md5Data, md5)
    ...
    if(md5Data == "Not Updated"):

        hashMD5 = "Not Updated"

    inodeDirBlockS = "empty"

    if(inodeDirBlock):
        inodeDirBlockS = "Block Pointers"
    else:
        inodeDirBlockS = "Extents"


    #we have more timestamp consistency checks here.  Limiting ts to dates 2021
    if((accessed < 1609459200) and (accessed > 946684800) and (modified > 946684800) and (changed > 946684800) and (changed < 1609459200) and (modified < 1609459200) and (deleted < 1609459200) and (createdTime < 1609459200) and ((deleted == 0) or ((deleted >= accessed) and (deleted >= changed) and (deleted >= modified) and (deleted > createdTime)))):
        
        if(md5):

            c.writerow([validInodeLoc, estInodeNum, estFilename,rcdInodeNum,rcdFN, fType, permString, userID, totSize, sectorCount, ATDT,nsA, CTDT,nsC, MTDT, nsM, DTDT, CRTDT, nsCr, fileVer, gID, linkCount, inodeDirBlockS, dataPtrs,offsetOff, hashMD5])

        else:

            c.writerow([validInodeLoc, estInodeNum, estFilename,rcdInodeNum,rcdFN, fType, permString, userID, totSize, sectorCount, ATDT,nsA, CTDT,nsC, MTDT, nsM, DTDT, CRTDT, nsCr, fileVer, gID, linkCount, inodeDirBlockS, dataPtrs,offsetOff])
    ...
    while((entryLen !=0) and stillMore):
        ...
        if((entryLen != 0)):
            ...
            if(stillMore):
                ...
                if(inodeNum not in inodeDict  and (trackBytes > 24 )):
...
def dir4CheckValidNonExt(mmapF, relativeOffsetInode, BGSize, bSize, partitionStart):
    ...
    #This is not extent, this is the first entry of the dir entry.
    if(bytesToDec(mmapF[dirBlockLocAdjusted + 4: dirBlockLocAdjusted + 4 + 3]) == 65548):
        return True
    else:
        #print("invalid!! NonExt")
        #print(relativeOffsetInode)
...
def dir4(mmapF, relativeOffsetInode, BGSize, bSize, partitionStart, inodeDict, fileVerDict, extFlag):
    ...
    #has this taken into account the offset???
    if(bytesToDec(mmapF[dirBlockLocAdjusted  + 4: dirBlockLocAdjusted  + 4 + 3]) == 65548):
        return [relativeOffsetInode, inodeNum]
...
def file4Validator(mmapF, relativeOffsetTS, BGSize, bSize, partitionStart, pFFShift, extFlag, magicNum, potentialUnusedZeros, validTimestamps, dirEntrySynchs, inodeDict, fileVerDict):
    ...
    if( (((extFlag & 0x08) == 0x08)  and    (magicNum == 62218)) or (((extFlag & 0x08) != 0x08) and ((bytesToDec(potentialUnusedZeros)) == 0)) ):
        ...
        if( ((totSize/512) > (sectorCount))  or ((totSize/512) < (sectorCount - 512)) ):
            return 0
        ...
        if ((pFFShift == 0x04) and (deleted == 0)):
            ...
            #Confusion with extent flag?
            if (  ((extFlag & 0x08) == 0x08) and (magicNum == 62218)):
...
def ExtInodeValidator(mmapF, timeList, BGSize, blockGroupTotal, bSize, partitionStart, inodeDict, fileVerDict):
    ...
    #The non-extent inodes are having some FP potential?
...
def main(timestamps, fileLoc, partitionStart, bSize, md5):
    ...
    if(md5):
        c.writerow(["Valid Inode Location", "Estimated Inode Number", "Estimated Filename", "Recorded Inode Number", "Recorded Filename", "Filetype", "Permissions", "User ID", "Total Size", "Sector Count", "Access Time", "Access Time ns", "Change Time", "Change Time ns", "Modified Time", "Modified Time ns", "Deleted Time", "Created Time", "Created Time ns", "File Version", "Group ID", "Link Count", "Extents or Pointers", "Data Pointers/Extents", "Is Inode offset off?", "MD5"])
    else:
        c.writerow(["Valid Inode Location", "Estimated Inode Number", "Estimated Filename", "Recorded Inode Number", "Recorded Filename", "Filetype", "Permissions", "User ID", "Total Size", "Sector Count", "Access Time", "Access Time ns", "Change Time", "Change Time ns", "Modified Time", "Modified Time ns", "Deleted Time", "Created Time", "Created Time ns", "File Version", "Group ID", "Link Count", "Extents or Pointers", "Data Pointers/Extents", "Is Inode offset off?"])


    #Read in list of timestamps, turn it into a list of ints.  Before, we were reading in timestamps prior to the
    #Partition under examination!
    timeList = []

    for line in open(timestamps):
        testTime = int(line.strip('\n'))
        if(testTime >= partitionStart):
            timeList.append(testTime)
    ...
    for i in validInodes:
        printInodes(mapF, q, i, dirEntrySynchronizers, BGSize, bSize, partitionStart, inodeDict, fileVerDict, c, md5)
...
def parse_arguments():
    ...
    parser.add_argument('-m', '--md5', action="store_true", help = "Option for hashing file contents (WIP)")
...
if __name__ == '__main__':
    ...
    main(arguments.timeFilePath, arguments.imagePath, arguments.readStart, arguments.blockSize, arguments.md5) 
